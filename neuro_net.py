...
class neuro_net:
    ...
    def train_nn_with_hidden_unit(self):
        for i in range(self.num_epochs):
            for j in range(self.train_size[0]):
                ...
                hidden_unit_input = [1] + self.get_sigmoid_list(hidden_net)
                ...
                delta_hidden_unit = np.dot(np.dot(hidden_unit_input, np.subtract(1, hidden_unit_input)), delta_output * self.hidden_layer)
                # print "delta hidden unit is {}".format(delta_hidden_unit)
    ...
    def test_with_hidden_unit(self):
        ...
        error = 0
        for j in range(self.train_size[0]):
            label = self.train_labels[j].item(0)
            train_sample = [1] + self.train_features[j,:].tolist()[0]
            hidden_net = np.matmul(self.input_layer, train_sample)
